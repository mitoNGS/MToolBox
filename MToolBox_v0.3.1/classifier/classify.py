...
def compare_mutations_regions(h_pos_list, s_pos_list, regions = None):
    ...
    if regions:
        for region in regions:
            ...
            h_pos_list_checked.extend([x for x in h_pos_list if x >= region_start and x <= region_end])
    else:
...
def align_sequences(rif, inc): #muscle
    muscle = subprocess.Popen(['./muscle -quiet'], shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)
...
class Classify(object):
    ...
    def prediction_sorting(self):
        ...
        if s == "complete":
            ...
            self.haplo_stats_sorted = OrderedDict(sorted(self.haplo_stats.items(), key=lambda x: x[1][0], reverse = True))
    ...
    def get_marges(self, marge_dict):
        ...
        self.marges = set([parse_marge.which_marge_lite(i, marge_dict) for i in best_list])
